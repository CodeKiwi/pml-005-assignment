...
Split into training, test and cross validation data sets. It was decided not to use K-Fold cross validation as this was decided to be overkill for the assignment. Using an 80% split yeilds 12k test cases with approx 3.5k samples in test and cross validation sets.
...
cross_val <- cross_val[-invalidColumns]
prediction_input <- prediction_input[-invalidColumns]
  
# Replace Divide by zero errors and empty cells with NA
training[training == "#DIV/0!" | training == ""] <- NA
cross_val[cross_val == "#DIV/0!" | cross_val == ""] <- NA
prediction_input[prediction_input == "#DIV/0!" | prediction_input == ""] <- NA
...
Given the selected features the pridicted OOB error rate is approximately 0.55%.
...
####Final result
The prediction accuracy of the final model was > 99.2%. This is a very positive result with an actual OOB error rate of 0.66%, marginally higher than the expected value of 0.55% but well within acceptible margins.  
...
####Predict test data
The final requirement of the assignment is to predict on a test set for submission to the Coursera site. This was completed (and passed 100%) with the function below. 
